...
\def\labelenumi{\arabic{enumi}.}
\item
  For any \(t\), \(N(t,t+7) = N(t+7) - N(t)\) has a Poisson distribution with mean \$ 7 \times 8 = 56\$. In any given period of 1 week,
  \[ \Pr(\text{no claim}) = e^{-56} \approx 0, \]
  and
  \[ \Pr(\text{at least two claims}) = 1 - e^{-56}  - (56)e^{-56}  = 1 - (57)e^{-56} \approx 1. \]
\item
  Each claim is independently ``male'' or ``female''. The required event happens if and only if, of the first 5 claims, at least 4 are claims from females. This follows because if the 4th claim from female occurs before or at the 5 claim, then it must have occurred before the 2nd claim from male Therefore, \(N_f | 5 \text{ claims} = N_f | N = 5 \sim \mathcal{B}(5, 3/4)\) and
  \[ 
  \begin{aligned}
...
\textbf{Note} It should be noted that the probability of getting \(k\) successes before the \(r\)th failure can be calculated by taking the sum (over \(j = 0,1,\ldots, r-1\)) of the probability of \(k-1\) successes and \(j\) failures followed by a success. This results in
...
\textbf{Solution:} Let \(T\) be the amount of time spending in the dentist office, which is exponentially distributed \(\text{Exp}(\lambda)\) with \(\lambda = 60\).

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \(\Pr( T > 60) = e^{-60 \lambda} = e^{-60 (1/40)} = e^{-1.5} = 0.2231.\)
\end{enumerate}
...
\item
  ...
  number of claims per year is \(\mathcal{Poisson}(0.1)\). \textbf{Suppose at time 0 all policyholders are at level 1.}
...
\includegraphics{SCMA469Bookdownproj_files/figure-latex/unnamed-chunk-19-1.pdf}
...
## 4 0.00000000 0.00467884 0.09048374 0.9048374
\end{verbatim}
...
\[\boldsymbol{\pi} = (0.0020032, 0.0144456, 0.0935973, 0.8899539),\]
...
\includegraphics{SCMA469Bookdownproj_files/figure-latex/unnamed-chunk-22-1.pdf}
...
\[ \Pr(X_2 = 2 | X_0 = 4) = (P^2)_{42} = 0.1875.\]
...
\[\Pr(X(2) = 0) = \frac{\exp(-(0.1)(2))((0.1)(2))^{0}  }{(0!)} = 0.8187308.\]
...
\begin{verbatim}
    1.  $\Pr(N(5) = 0) = \frac{\exp(-(2)(5))((2)(5))^{0}  }{(0!)} = \ensuremath{4.539993\times 10^{-5}}$.
...
\end{aligned}
\]
4. Each interarrival time has exponential distribution \(\text{Exp}(2)\). The sum of 5 independent exponentially distribution random variables, denoted by \(Z\), has gamma distribution \(\mathcal{G}(5,2)\)
The probability that a time longer than 5 minutes will pass between one voucher being given out and the next is
\[ \Pr(Z > 5) = \Pr(N(5) \le 4) = 0.0292527.\]

Here we use \(N(5) \sim \text{Poisson}(5\times 2)\). Alternatively, the following command in R produces the required result \texttt{pgamma(5,\ shape\ =\ 5,\ rate\ =\ 2,\ lower.tail\ =\ FALSE)}.
...
\end{verbatim}
  ...
  \[\Pr(N_S(1) = 10) = \frac{\exp(-(20)(0.25))((20)(0.25))^{10}  }{(10!)} = 0.0181328.\]
...
\end{verbatim}
  ...
  \end{aligned}
  \]
  The probability above can be calculated using the following command in R \texttt{pbinom(29,60,1/4)}.
\item
  Let \(X_1, X_2, \ldots\)\$ be an i.i.d. sequence where \(X_i\) is the number of claims filed after the \(i\)th flood. Let \(T\) denote the total number of claims filed. Then
...
Note that the distribution of \(T\) is said to have a compound Poisson distribution. For more details about how to compute \(\text{E}(T)\) and \(\text{Var}(T)\), please refer to the following website: \url{https://pairote-sat.github.io/SCMA470/collective-risk-model.html\#compound-poisson-distributions}.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Suppose that we observe life \(i\) at exact age 74 years and 3 months.
  ...
  \def\labelenumii{\arabic{enumii}.}
  \item
    Calculate the probability function of \(D_i\), i.e.~calculate
    \(\Pr(D_i = 0)\) and \(\Pr(D_i = 1)\).
  \item
    Calculate \(\textrm{E}[D_i]\).
  \item
    Calculate the probability density/mass function of \(V_i\) (Hint:
...
\item
  ...
  \def\labelenumii{\arabic{enumii}.}
  \item
    explain why the following expression holds:
    \[\int_0^{b_i - a_i} e^{-\mu v_i} \mu \, \text{d}v_i + e^{-\mu(b_i - a_i)} = 1.\]
  \item
    Differentiating the above expression with respect to \(\mu\), show
    that \[\textrm{E}[ D_i - \mu V_i] = 0 .\]
  \item
    Differentiating the above expression twice with respect to
...
\item
  1300 lives aged between 70 and 71 have been observed. We wish to
...
\item
  Consider the following mortality data on ten lives all aged between
  \(75\) and \(76\).
  ...
  \def\labelenumii{\arabic{enumii}.}
  \item
    Using the Markov model, estimate the force of mortality
    \(\mu_{75}\) assuming that it is constant from \(75\) to \(76\).
  \item
    Estimate the variance of \(\hat{\mu}\).
  \item
    Construct the 95\% confidence interval for the force of
    mortality.
  \item
    Estimate \(\hat{q}_{75} = {}_{1}\hat{q}_{75}\), the probability of
    a life aged (75) dying within one year.
  \item
    Use the \(\Delta-\)method to estimate the variance of
    \(\hat{q}_{75}\).
  \end{enumerate}
\end{enumerate}
...
\begin{figure}
\centering
\includegraphics{SCMA469Bookdownproj_files/figure-latex/tikz-ex0-1.png}
\caption{\label{fig:tikz-ex0}The 3-state model of terminal illness}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{SCMA469Bookdownproj_files/figure-latex/tikz-ex1-1.png}
\caption{\label{fig:tikz-ex1}A model of the mortality of two lives}
\end{figure}
...
\tightlist
\item
  A group of lives who hold health insurance policies can be
...
\begin{figure}
\centering
\includegraphics{SCMA469Bookdownproj_files/figure-latex/tikz-ex2-1.png}
\caption{\label{fig:tikz-ex2}A Markov model for health insurance policies}
\end{figure}
...
\tightlist
\item
  An actuary wishes to study a model in which states relate to marital
...
\tightlist
\item
  The transition rate matrix \(Q\) of this Markov model is
  \[
    Q = \begin{bmatrix}
    -(\lambda + \mu) & \lambda & \mu   \\
    0 & -\nu & \nu   \\
...
\begin{figure}
\centering
\includegraphics{SCMA469Bookdownproj_files/figure-latex/tikz-ex6-1.png}
\caption{\label{fig:tikz-ex6}The 5-state model of maritial status}
\end{figure}

\hypertarget{tutorial-7}{%
\section{Tutorial 7}\label{tutorial-7}}

\textbf{Questions are modified from the past CT4 exams from IFoA.}
...
\textbf{Reasonableness of the result.}
...
\textbf{Comply with professional guidelines and the regulatory environment.}
