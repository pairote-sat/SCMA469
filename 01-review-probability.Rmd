...
::: {.example}
*Let $N$ denote the number of claims which arise up to a
...
::: {.example}
*Let $X$ denote the claim sizes in a given time period.
...
ggplot(df) + 
    stat_function(aes(x),fun=dweibull,args = list(shape = 1, scale = 1))  + 
    labs(x = "x", y = "f(x)", 
       title = "Weibull Distribution With Shape & Scale Parameters = 1") 
```
...
::: {.example}
*Suppose in a particular study area, the vaccination rate
for the yearly flu virus is 70%. In addition, 10% of those vaccinated still get the flu that year. Calculate the conditional
probability of someone getting the flu in this area given that the
person was vaccinated.*
:::

::: {.example}
*You are an invester buying shares of a company. You have
